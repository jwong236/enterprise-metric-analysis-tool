...
from models import (
    Team, Service, Repository, PullRequest, BlockedTask,
    DeploymentFrequency, LeadTimeForChange, OpenIssueBug,
    RefinementChange, RetroMood
)
                ...
                try:
                    ...
                    for _ in range(deployment_count):
                        deployment = DeploymentFrequency(
                            deployment_id=generate_id(DEPLOY_PREFIX, id_counters['deploy']),
                            date=day,
                            team_id=random.choice(teams).id,
                            service_id=random.choice(services).id
                        )
                    ...
                    for _ in range(daily_changes):
                        refinement = RefinementChange(
                            refinement_id=generate_id(CHANGE_PREFIX, id_counters['change']),
                            date=day,
                            team_id=random.choice(teams).id,
                            service_id=random.choice(services).id
                        )
